...
import { ConditionRating, InventoryItem } from '@/types';
...
export default function InventoryScreen() {
  const { inventoryItems, tenancyStartDate } = currentTenancy;
  const rooms = [...new Set(inventoryItems.map((i) => i.room))];
  const totalPhotos = inventoryItems.filter((i) => i.photoTaken).length;
  const moveInDate = new Date(tenancyStartDate).toLocaleDateString('en-GB', {
    day: 'numeric',
    month: 'long',
    year: 'numeric',
  });
    ...
    <SafeAreaView style={styles.safeArea}>
      <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
        <View style={styles.header}>
          ...
          <Text style={styles.subtitle}>Recorded {moveInDate}</Text>
        </View>
